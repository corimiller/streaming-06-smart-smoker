"""
    This program will listen continuously to for the smart smoker producer program.  
    It is listening for the 3 queues of Smoker, Food A and Food B.
    An alert will trigger for temp decrease of 15 in 2.5 minutes for smoker or less then 1 degree
    increase for foods over 10 minutes

    Got help from Ryan Smith and A. Ledgerwood
...
def process_temperature(body, temperature_name, temperature_deque, alert_threshold):
    ...
    try:
        ...
        if len(temperature_deque) == temperature_deque.maxlen:
            ...
            time_diff = (temperature_deque[-1][0] - temperature_deque[0][0]).total_seconds() / 60.0  
            # Convert to minutes
